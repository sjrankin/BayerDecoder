                        ...
                        uint2 gid [[thread_position_in_grid]])
{
    ...
    //When running on macOS, the Y order is reversed from what iOS expected so for that reason, the
    //position of the Y coordinate is reversed here.
    uint2 FinalPosition = uint2(gid.x, OutTexture.get_height() - gid.y);
    OutTexture.write(Final, FinalPosition);
}
